...
function Model:createVAE(X)
  ...
  self.encoder:add(nn.Linear(128, 100))
  self.encoder:add(nn.BatchNormalization(100))
  self.encoder:add(nn.ReLU(true))
  -- Create latent Z parameter layer
  local zLayer = nn.ConcatTable()
  zLayer:add(nn.Linear(100, self.zSize)) -- Mean μ of Z
  zLayer:add(nn.Linear(100, self.zSize)) -- Log variance σ^2 of Z (diagonal covariance)
  ...
  self.decoder:add(nn.Linear(self.zSize, 100))
  self.decoder:add(nn.BatchNormalization(100))
  self.decoder:add(nn.ReLU(true))
  self.decoder:add(nn.Linear(100, 128))
