...
#pragma omp parallel for
  ...
  my_start_row_index_ = 0;
//#pragma omp parallel for reduction(+: my_start_row_index_)
  ...
  while (getline(fin2, line)) {  // Each line is a training document.
    if (line.size() > 0 &&      // Skip empty lines.
      ...
      local_rows_.push_back(row);  //存储在二维向量local_rows
      ++i;
    }
  }
...
//
      //std::cout<<cluster_centers_[i][0]<<' ';
}    
  fout.close();

//std::cout<<num_total_rows_<<std::endl;
//std::cout<<local_rows_[0][0]<<std::endl;
//std::cout<<local_rows_[0][1]<<std::endl;
//std::cout<<cluster_centers_[1][1]<<std::endl;
    }
...
for (int i = 0; i < num_clusters_; ++i) {
    ...
    if (myid_ == 0) {
      LOG(INFO) << "Number of non-empty clusters is " << count_not_zero_ << std::endl;
...
double KMeans::DotProduct(const vector<double>& v1, const double* v2) {
  double norm_sq = 0.0;
//#pragma omp parallel for reduction(+:norm_sq) firstprivate(norm_sq) 
...
//
void KMeans::calc_likelihood(vector<int> local_memberships,vector<double> cluster_centers){
  int i = 0;
  double yc, xc;
  double new_likelihood = 0;
  while(i<num_total_rows_){
    if(i<my_start_row_index_ || i>=my_start_row_index_+row_count_of_each_proc_[myid_]){
        ++i;
        continue;
}
int j = local_memberships[i];
xc = cluster_centers[j];
yc = cluster_centers[j+1];
new_likelihood += (local_rows_[i][0]-xc)*(local_rows_[i][0]-xc)+(local_rows_[i][1]-yc)*(local_rows_[i][1]-yc);
++i;
}
MPI_Allreduce(&new_likelihood,
              &likelihood,
              1,
              MPI_DOUBLE,
              MPI_SUM,MPI_COMM_WORLD);	
}
void KMeans::calc_BIC(int num_total_rows, int count_not_zero){
calc_likelihood(local_memberships_, cluster_centers_storage_); 
BIC =  likelihood + std::log(num_total_rows) * 2 * count_not_zero;
//AIC = likelihood + 
}
}  // namespace kmeanst 
using namespace kmeanst;
using namespace std;
 ...
 // start = time(0);
  ...
  for (int i = 1; i < argc; ++i) {
    if (0 == strcmp(argv[i], "--num_clusters")) {
      ...
      ++i;
    } else if (0 == strcmp(argv[i], "--init_method")) {
  ...
  double old_BIC = numeric_limits<double>::min();
  double new_BIC = numeric_limits<double>::max();
  double limits_BIC = 1e-3;
  int num_of_clusters = FLAGS_num_clusters;
  KMeans kmeans;
  ...
  new_BIC = kmeans.get_BIC();
  while(new_BIC - old_BIC>limits_BIC){
    ++num_of_clusters;
    kmeans.DoKMeans(num_of_clusters, FLAGS_initialization_method,
                    FLAGS_kmeans_loop, FLAGS_kmeans_threshold);
    old_BIC = new_BIC;
    new_BIC = kmeans.get_BIC();
}
